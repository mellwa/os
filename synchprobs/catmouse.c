...
#if OPT_A1
bool* volatile  bowls;
int volatile numEatingCats;
int volatile numEatingMice;
int volatile numWaitingCats;
int volatile numWaitingMice;
bool volatile cat_eating;
bool volatile mouse_eating;
bool volatile all_cats_done;
bool volatile all_mice_done;
struct lock* block;
struct lock* block2;
struct cv* cv_cat;
struct cv* cv_mouse;
               ...
               unsigned long catnumber)
{
  ...
  for(i=0;i<NumLoops;i++) {
...
#if OPT_A1
    lock_acquire(block);
      while(numEatingCats == NumBowls || mouse_eating){
          numWaitingCats++;
          cv_wait(cv_cat,block);
          numWaitingCats--;
      }
      cat_eating = true;
      numEatingCats++;
    ...
    lock_acquire(block2);
      ...
      if(numEatingCats == 0){
          cat_eating = false;
          cv_broadcast(cv_mouse,block2);
          if(all_mice_done){
              cv_broadcast(cv_cat,block2);
          }
          if((NumLoops-1)  == i && numWaitingCats==0){
              all_cats_done = true;
          }
      }
    bowls[bowl-1]=false;
  ...
  for(i=0;i<NumLoops;i++) {
...
#if OPT_A1
      lock_acquire(block);
      while(numEatingMice==NumBowls || cat_eating){
          numWaitingMice++;
          cv_wait(cv_mouse,block);
          numWaitingMice--;
      }
      mouse_eating = true;
      numEatingMice++;
    ...
    lock_acquire(block2);
      ...
      if(numEatingMice == 0){
          ...
          if((NumLoops-1) == i && numWaitingMice==0){
              all_mice_done = true;
          }
      }
         ...
         char ** args)
{
...
#if OPT_A1
    cv_cat = cv_create("cat");
    cv_mouse = cv_create("mouse");
  block = lock_create("BLOCK");
  block2 = lock_create("BLOCK2");
    all_cats_done = false;
    all_mice_done = false;
    if(NumMice == 0){
        all_mice_done = true;
  ...
  bowls = kmalloc(NumBowls * sizeof(bool));
    ...
    //for(i=0;i<NumBowls;i++){
    //    bowl_lock = kmalloc(NumBowls* sizeof(struct lock));
